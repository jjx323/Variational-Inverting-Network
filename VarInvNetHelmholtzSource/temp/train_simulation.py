...
import fenics as fe
from ModelN import vec2matrix, matrix2vec 
...
mesh = fe.UnitSquareMesh(args.patch_size-1, args.patch_size-1)
V = fe.FunctionSpace(mesh, 'P', 1)
ll = args.patch_size*args.patch_size
Id_matrix = sparse.coo_matrix(sparse.eye(ll))

# convert sparse matrix in scipy to sparse tensor in PyTorch
# row = torch.from_numpy(Id_matrix.row.astype(np.int64)).to(torch.long)
# col = torch.from_numpy(Id_matrix.col.astype(np.int64)).to(torch.long)
# edge_index = torch.stack([row, col], dim=0)
# #Presuming values are floats, can use np.int64 for dtype=int8
# val = torch.from_numpy(Id_matrix.data.astype(np.float64)).to(torch.float)
# Id_tensor = torch.sparse.FloatTensor(edge_index, val, torch.Size(Id_matrix.shape))

def forward_op(z):
    if z.is_cuda:
        z_np = matrix2vec(z.detach().cpu().numpy(), V)
    else:
        z_np = matrix2vec(z.detach().numpy(), V)
    z_np = z_np.reshape(-1, ll)
    out = (z_np@Id_matrix).reshape(-1, 3, ll)
    out = vec2matrix(out, V)
    out = torch.from_numpy(out).to(device=z.device).type(dtype=z.dtype)
    return out
#forward_op = None
    ...
    for epoch in range(args.epoch_start, args.epochs):
        ...
        for ii, data in enumerate(data_loader[phase]):
            ...
            if args.learn_type == 'residual':
                ...
                im_denoise = torch.clamp(phi_Z[:, :C, ].detach().data, 0.0, 1.0)
            else:
...
def main(loss_fn_type='residual'):
    # build the model
